...
from ekosis.queues.pending_queue               import PendingQueue, PendingEntry, ErrorEntry
...
def _make_entry(item_uuid: uuid.UUID, data: str, metadata: dict = None) -> PendingEntry:
    return PendingEntry(
        uid      = str(item_uuid),
        data     = data,
        retries  = 0,
...
async def test_pending_metadata_survives_memory_push_pop():
    item_uuid = _make_uid()
    await QUEUE.push_pending(item_uuid, "data", 0, {"trace_id": 42, "span_id": 7})
...
async def test_pending_metadata_survives_sqlite_page_boundary():
    target_uid = _make_uid()
    # fill past two full pages (page_size=10) to force target into SQLite
    for _ in range(21):
        await QUEUE.push_pending(_make_uid(), "filler", 0, {})
    await QUEUE.push_pending(target_uid, "target_data", 0, {"trace_id": 99, "span_id": 3})
    # drain until we reach target
    found = None
    while QUEUE.pending_q.size() > 0:
        entry = await QUEUE.pending_q.pop()
        if entry and uuid.UUID(entry.uid) == target_uid:
            found = entry
            break
...
async def test_error_metadata_survives_push_pop():
    item_uuid = _make_uid()
    await QUEUE.push_error(item_uuid, "data", "test failure", {"trace_id": 11, "span_id": 5})
...
async def test_metadata_survives_move_all_error_to_pending():
    item_uuid = _make_uid()
    await QUEUE.push_error(item_uuid, "data", "failure", {"trace_id": 20, "span_id": 8})
...
async def test_metadata_survives_move_one_error_to_pending():
    item_uuid = _make_uid()
    await QUEUE.push_error(item_uuid, "data", "failure", {"trace_id": 30, "span_id": 9})
    await QUEUE.move_one_error_to_pending(item_uuid)
    result = await QUEUE.pending_q.pop()
...
async def test_push_pending_no_metadata_defaults_empty():
    item_uuid = _make_uid()
    await QUEUE.push_pending(item_uuid, "data")
    result = await QUEUE.pending_q.pop()
...
async def test_push_error_no_metadata_defaults_empty():
    item_uuid = _make_uid()
    await QUEUE.push_error(item_uuid, "data", "reason")
    result = await QUEUE.error_q.pop()
    assert result.metadata == {}
...
class _MetaMiddleware(BufferedMiddlewareBase):
    ...
    async def before_push(self, uid: uuid.UUID, dto, **kwargs) -> dict:
...
class _TrackingMiddleware(BufferedMiddlewareBase):
    ...
    async def before_process(self, uid, dto, metadata, retries):
        self.before_process_calls.append((uid, retries))

    async def after_process(self, uid, dto, metadata, success):
        self.after_process_calls.append((uid, success))
...
async def test_collect_push_metadata_merges_dicts():
    ...
    result = await manager.collect_push_metadata(_make_uid(), None)
...
async def test_before_process_called_with_correct_args():
    ...
    uid      = _make_uid()
    metadata = {"trace_id": 5}
    await manager.run_before_process(uid, None, metadata, retries=2)
    assert tracker.before_process_calls == [(uid, 2)]
...
async def test_after_process_called_with_correct_args():
    ...
    uid      = _make_uid()
    metadata = {"trace_id": 5}
    await manager.run_after_process(uid, None, metadata, success=True)
    assert tracker.after_process_calls == [(uid, True)]
...
async def test_before_and_after_process_called_in_order():
    ...
    uid = _make_uid()
    await manager.run_before_process(uid, None, {}, 0)
    await manager.run_after_process(uid, None, {}, True)
...
async def test_reprocess_does_not_call_before_push():
    ...
    class _PushTracker(BufferedMiddlewareBase):
        async def before_push(self, uid, dto, **kwargs) -> dict:
            push_called.append(uid)
            return {}
    ...
    await QUEUE.push_error(item_uuid, "data", "failure", {"trace_id": 77, "span_id": 4})
