...
# ConfigLogging
# --------------------------------------------------------------------------------
# TODO: Set this up through environment variables as well.
...
class AppConfiguration(metaclass=SingletonType):
    ...
    def dict(self):
        return {
            ...
            "tcp"            : None if self.tcp is None else self.tcp.dict(),
            "udp"            : None if self.udp is None else self.udp.dict(),
            "uds"            : None if self.uds is None else self.uds.dict(),
            "stats_keeper"   : self.stats_keeper.dict(),
