...
namespace {
    ...
    nlohmann::json handler_info_json(BufferedHandlerInterface& handler) {
        return {
            ...
            {
                "database_sizes", {
                    {"pending", handler.pending_queue_size()},
                    {"error"  , handler.error_queue_size()}
                }
            },
        };
    }
    ...
    // -------------------------------------------------------------------------
    std::shared_ptr<BufferedHandlerInterface> find_handler(
    ...
    nlohmann::json not_found(const std::string& route_key) {
        return {
            ...
            {"message"     , "No queue for route key: [" + route_key + "]"}
        };
    }

    // -------------------------------------------------------------------------
    nlohmann::json with_handler_info(
        BufferedHandlerInterface& handler,
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: Paused RECEIVING."
                )
            };
    });
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: UN-Paused RECEIVING."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: Paused PROCESSING."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: UN-Paused PROCESSING."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: UN-Paused ALL."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: Error database entries, moved to incoming database."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_handler_info(
                    *handler,
                    "Buffered Endpoint[" + route_key + "]: Error database cleared."
                )
            };
        }
    );
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            if (!result) {
                return RequestDTO{{
                    {"queue_data"  , nullptr},
                    {"request_data", nullptr},
                    {"message"     , "No request with span-key [" + span_key_string + "] in error queue for route key: [" + route_key + "]"}
                }};
            }
