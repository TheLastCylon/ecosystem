...
using SenderRegistry = std::unordered_map<std::string, std::shared_ptr<BufferedSender>>;
    ...
    nlohmann::json sender_info_json(BufferedSender& sender) {
        return {
            ...
            {
                "database_sizes", {
                    {"pending", sender.pending_queue_size()},
                    {"error"  , sender.error_queue_size()}
                }
            },
        };
    }
    ...
    // -------------------------------------------------------------------------
    std::shared_ptr<BufferedSender> find_sender(
    ...
    nlohmann::json not_found(const std::string& route_key) {
        return {
            ...
            {"message"     , "No buffered sender with route key: [" + route_key + "]"}
        };
    }
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_sender_info(
                    *sender,
                    "Buffered Sender[" + route_key + "]: Paused SEND PROCESS."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_sender_info(
                    *sender,
                    "Buffered Sender[" + route_key + "]: UN-Paused SEND PROCESS."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_sender_info(
                    *sender,
                    "Buffered Sender[" + route_key + "]: Error database entries, moved to Retry database."
                )
            };
        }
    );
    ...
    router.register_endpoint(
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            return RequestDTO{
                with_sender_info(
                    *sender,
                    "Buffered Sender[" + route_key + "]: Error database cleared."
                )
            };
        }
    );
        ...
        [&registry](SpanKey, RequestDTO& dto) -> RequestDTO {
            ...
            if (!result) {
                return RequestDTO{{
                    {"queue_data"  , nullptr},
                    {"request_data", nullptr},
                    {"message"     , "No request with span-key [" + span_key_string + "] in buffered sender error database, for route key: [" + route_key + "]"}
                }};
            }
