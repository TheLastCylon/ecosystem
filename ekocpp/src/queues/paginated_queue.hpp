...
class PaginatedQueue
{
    public:
        ...
        std::optional<QueuedType> inspect_span_key(const SpanKey& span_key) const {
            if (auto found = front_page_->inspect_entry(span_key)) {
                return found;
            }

            if (auto found = back_page_->inspect_entry(span_key)) {
                return found;
            }
        ...
        std::optional<QueuedType> pop_span_key(const SpanKey& span_key) {
            if (auto found = front_page_->pop_entry(span_key)) {
                return found;
            }

            if (auto found = back_page_->pop_entry(span_key)) {
                return found;
            }
        ...
        std::vector<std::string> get_first_x_span_keys(size_t how_many) const
        {
            ...
            if (result.size() >= how_many || back_page_ == front_page_) {
                return result;
            }
