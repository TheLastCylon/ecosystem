...
// --------------------------------------------------------------------------------
asio::awaitable<ResponseDTO> ServerBase::route_request(
...
// --------------------------------------------------------------------------------
ResponseDTO ServerBase::build_parsing_error_response(
    const SpanKey&        span_key,
    const std::exception& error
) const {
    return ResponseDTO{
        span_key,
        static_cast<int>(Status::PROTOCOL_PARSING_ERROR),
        error.what()
    };
}

// --------------------------------------------------------------------------------
asio::awaitable<std::vector<uint8_t>> ServerBase::process_request(
