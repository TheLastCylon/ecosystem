...
async def eco_buffered_sender_errors_get_first_10(dto: QManagementRequestDto, **kwargs) -> PydanticBaseModel:
    buffered_sender_keeper = BufferedSenderKeeper()
    span_key_list          = await buffered_sender_keeper.get_first_x_error_span_keys(dto.queue_route_key)
    if span_key_list is None:
        return QManagementResponseDto(message=f"No buffered sender with route key: [{dto.queue_route_key}]")

    return QManagementResponseDto(
        queue_data = span_key_list,
        message    = f"Buffered Sender[{dto.queue_route_key}]: Retrieved first 10 span-keys for error database."
    )
...
async def eco_buffered_sender_errors_reprocess_one(dto: QManagementItemRequestDto, **kwargs) -> PydanticBaseModel:
    buffered_sender_keeper = BufferedSenderKeeper()
    queue_info_dict        = await buffered_sender_keeper.reprocess_error_queue_span_key(dto.queue_route_key, dto.span_key)

    if queue_info_dict is None:
        return QManagementResponseDto(message=f"No buffered sender with route key: [{dto.queue_route_key}]")
    elif not queue_info_dict:
        return QManagementResponseDto(message=f"No request with span-key [{dto.span_key}] in buffered sender error database, for route key: [{dto.queue_route_key}]")

    return QManagementResponseDto(
        queue_data   = make_buffered_sender_information_dto(dto.queue_route_key, queue_info_dict),
        request_data = queue_info_dict["request_data"],
        message      = f"Buffered Sender[{dto.queue_route_key}]: Error database entry[{dto.span_key}] moved to Retry database."
    )
...
async def eco_buffered_sender_errors_pop_request(dto: QManagementItemRequestDto, **kwargs) -> PydanticBaseModel:
    buffered_sender_keeper = BufferedSenderKeeper()
    queue_info_dict        = await buffered_sender_keeper.pop_request_from_error_queue(dto.queue_route_key, dto.span_key)
    if queue_info_dict is None:
        return QManagementResponseDto(message=f"No buffered sender with route key: [{dto.queue_route_key}]")
    elif not queue_info_dict:
        return QManagementResponseDto(message=f"No request with span-key [{dto.span_key}] in buffered sender error database, for route key: [{dto.queue_route_key}]")
    ...
    return QManagementResponseDto(
        queue_data   = make_buffered_sender_information_dto(dto.queue_route_key, queue_info_dict),
        request_data = queue_info_dict["request_data"],
        message      = f"Buffered Sender[{dto.queue_route_key}]: POPPED Error database entry[{dto.span_key}]."
    )
...
async def eco_buffered_sender_errors_inspect_request(dto: QManagementItemRequestDto, **kwargs) -> PydanticBaseModel:
    buffered_sender_keeper = BufferedSenderKeeper()
    queue_info_dict        = await buffered_sender_keeper.inspect_request_in_error_queue(dto.queue_route_key, dto.span_key)
    if queue_info_dict is None:
        return QManagementResponseDto(message=f"No buffered sender with route key: [{dto.queue_route_key}]")
    elif not queue_info_dict:
        return QManagementResponseDto(message=f"No request with span-key [{dto.span_key}] in buffered sender error database, for route key: [{dto.queue_route_key}]")
    ...
    return QManagementResponseDto(
        queue_data   = make_buffered_sender_information_dto(dto.queue_route_key, queue_info_dict),
        request_data = queue_info_dict["request_data"],
        message      = f"Buffered Sender[{dto.queue_route_key}]: INSPECTING Error database entry[{dto.span_key}]."
    )
