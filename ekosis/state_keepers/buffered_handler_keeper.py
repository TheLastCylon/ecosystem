...
# --------------------------------------------------------------------------------
class BufferedHandlerKeeper(metaclass=SingletonType):
    __buffered_handlers: Dict[str, BufferedHandler] = {}

    def __init__(self):
        pass

    # --------------------------------------------------------------------------------
    def add_buffered_handler(self, buffered_handler: BufferedHandler):
        self.__buffered_handlers[buffered_handler.get_route_key()] = buffered_handler
    ...
    async def __get_queue_information(self, route_key: str) -> Dict[str, Any]:
        return {
            "pending"          : await self.__buffered_handlers[route_key].pending_queue_size(),
            "error"            : await self.__buffered_handlers[route_key].error_queue_size(),
            "receiving_paused" : self.__buffered_handlers[route_key].is_receiving_paused(),
            "processing_paused": self.__buffered_handlers[route_key].is_processing_paused()
        }
    ...
    async def pause_receiving_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].pause_receiving()
        return await self.__get_queue_information(route_key)

    # --------------------------------------------------------------------------------
    async def pause_processing_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].pause_processing()
        return await self.__get_queue_information(route_key)

    # --------------------------------------------------------------------------------
    async def pause_all_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].pause_receiving()
        self.__buffered_handlers[route_key].pause_processing()
    ...
    # --------------------------------------------------------------------------------
    async def unpause_receiving_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].unpause_receiving()
        return await self.__get_queue_information(route_key)

    # --------------------------------------------------------------------------------
    async def unpause_processing_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].unpause_processing()
        return await self.__get_queue_information(route_key)

    # --------------------------------------------------------------------------------
    async def unpause_all_for_buffered_handler(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        self.__buffered_handlers[route_key].unpause_receiving()
        self.__buffered_handlers[route_key].unpause_processing()
    ...
    async def reprocess_error_queue(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        await self.__buffered_handlers[route_key].reprocess_error_queue()
        return await self.__get_queue_information(route_key)
    ...
    async def clear_error_queue(self, route_key: str):
        if route_key not in self.__buffered_handlers.keys():
            return None
        await self.__buffered_handlers[route_key].error_queue_clear()
        return await self.__get_queue_information(route_key)
